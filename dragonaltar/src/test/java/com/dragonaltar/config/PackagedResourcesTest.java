...
class PackagedResourcesTest {
	private static final List<String> CONFIGS = List.of("config.yml", "messages.yml", "altar.yml", "ritual.yml",
			"abilities.yml", "animations.yml");
	...
	void playerFacingResourcesContainNoLongDashes() throws Exception {
		for (String name : CONFIGS)
			assertNoLongDash(load(name).getValues(true), name);
	}

	@Test
	void jarContainsTheLicenseAndOwnerNotice() throws Exception {
		try (InputStream stream = PackagedResourcesTest.class.getClassLoader()
				.getResourceAsStream("META-INF/LICENSE.md")) {
	...
	void packagedConfigsExcludeRemovedSettings() throws Exception {
		...
		for (String path : List.of("player-only", "setup-incomplete", "event-recovery-required", "ritual-error",
				"event-started", "soul-bound-equip-denied"))
		...
		for (var entry : values.entrySet()) {
			if (entry.getValue() instanceof String text)
				assertFalse(text.contains("—") || text.contains("–"),
						name + " contains a long dash at " + entry.getKey());
		}
	}
