...
public final class ScaledEnderDragonIntegration {
	...
	public void observeCommand(String command) {
		...
		if (normalized.equals("sed kill") || normalized.startsWith("sed kill "))
			sedKillObservedAt = System.currentTimeMillis();
	}
	public String completionMethod() {
		if (System.currentTimeMillis() - sedKillObservedAt <= 10_000) {
			sedKillObservedAt = 0;
			return "SED_KILL";
		}
		return "COMBAT";
