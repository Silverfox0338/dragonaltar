...
public final class ConfirmationService {
	...
	ConfirmationService(SecureRandom random, Clock clock) {
	...
	public synchronized String issue(UUID sender, String operation, List<String> arguments, Duration ttl) {
		...
		pending.put(sender,
				new Confirmation(token.toString(), operation, List.copyOf(arguments), Instant.now(clock).plus(ttl)));
	...
	public synchronized boolean consume(UUID sender, String token, String operation, List<String> arguments) {
		...
		return c != null && Instant.now(clock).isBefore(c.expires()) && c.token().equalsIgnoreCase(token)
				&& c.operation().equals(operation) && c.arguments().equals(arguments);
	}
	public synchronized boolean cancel(UUID sender) {
		return pending.remove(sender) != null;
	}
