...
public final class AuditService implements AutoCloseable {
	...
	private final java.util.concurrent.ExecutorService writer = java.util.concurrent.Executors
			.newSingleThreadExecutor(r -> {
				Thread thread = new Thread(r, "DragonAltar-Audit");
	...
	public synchronized void record(String action, String actor, String detail) {
		...
		String line = DateTimeFormatter.ISO_INSTANT.format(Instant.now()) + "\t" + action + "\t" + actor + "\t"
		...
		writer.execute(() -> {
			try {
				...
				plugin.getLogger().log(Level.SEVERE, "Could not write audit log", e);
			}
		});
	}
	@Override
