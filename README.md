...
DragonAltar 1.4.16 targets Paper 1.21.4 and Java 21.
...
2. Copy `DragonAltar-1.4.16.jar` into the server's `plugins` directory.
...
The release JAR is written to `target/DragonAltar-1.4.16.jar`. Maven compiles with `--release 21`, runs the JUnit suite, filters only `plugin.yml`, and produces deterministic archive timestamps.
