...
final class SoulHistoryEntryTest {
	...
	void parsesLegacyLineageWithoutMigration() {
		...
		SoulHistoryEntry entry = SoulHistoryEntry
				.parse("2026-07-29T05:14:00Z|" + oldHolder + "|" + newHolder + "|PVP_INHERITANCE");
	...
	void parsesMotherSoulCallerMetadata() {
		...
		SoulHistoryEntry entry = SoulHistoryEntry
				.parse("2026-07-29T05:14:00Z|-|MOTHER_SOUL_LIMBO|DRAGONBORN_CALLER_BACKFIRE;callers=" + callerA + ","
						+ callerB);
	...
	void parsesARelevantKiller() {
		...
		SoulHistoryEntry entry = SoulHistoryEntry
				.parse("2026-07-29T05:14:00Z|" + victim + "|DISABLED|DRAGONBORN_KILLER_DORMANT;killer=" + killer);
	...
	void retainsPrivateAdminProvenanceButPublicTextIsNeutral() {
		...
		SoulHistoryEntry entry = SoulHistoryEntry
				.parse("2026-07-29T05:14:00Z|" + oldHolder + "|" + newHolder + "|ADMIN_FORCE_TRANSFER;admin=" + admin);
	...
	void parsesPrivateCustodyParticipants() {
		...
		SoulHistoryEntry entry = SoulHistoryEntry.parse("2026-07-29T05:14:00Z|" + oldHolder + "|" + adminHolder
	...
	void olderEntriesHideAnAdminWhoWasAlsoTheHolder() {
		...
		UUID adminHolder = UUID.randomUUID();
		SoulHistoryEntry entry = SoulHistoryEntry
				.parse("2026-07-29T05:14:00Z|" + player + "|" + adminHolder + "|ADMIN_TRANSFER;admin=" + adminHolder);
