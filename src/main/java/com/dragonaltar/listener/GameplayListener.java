...
public final class GameplayListener implements Listener {
    private final DragonAltarPlugin plugin; private final DragonEventManager event; private final DragonSoulService souls;
    private final DragonbornService dragonborn; private final AbilityService abilities; private final EligibilityService eligibility;
    private final CombatTagService combatTags;
    private final Set<String> scheduledTransfers=new HashSet<>();
    public GameplayListener(DragonAltarPlugin plugin, DragonEventManager event, DragonSoulService souls, DragonbornService dragonborn,
                            AbilityService abilities, EligibilityService eligibility, CombatTagService combatTags) {
        this.plugin=plugin; this.event=event; this.souls=souls; this.dragonborn=dragonborn; this.abilities=abilities; this.eligibility=eligibility;this.combatTags=combatTags;
    }
    @EventHandler public void spawn(CreatureSpawnEvent e) { if (e.getEntity() instanceof EnderDragon d&&!event.isTestDragon(d))event.trackVanillaSpawn(d,e.getSpawnReason()); }
    @EventHandler(ignoreCancelled=true,priority=EventPriority.HIGHEST) public void protectCrystal(EntityDamageEvent e){if(e.getEntity() instanceof EnderCrystal crystal&&event.isEventCrystal(crystal))e.setCancelled(true);}
    @EventHandler(ignoreCancelled=true,priority=EventPriority.HIGHEST) public void protectDisplay(EntityDamageEvent e){if(plugin.displays().owns(e.getEntity()))e.setCancelled(true);}
    @EventHandler(ignoreCancelled=true,priority=EventPriority.HIGHEST) public void protectDisplayInteraction(org.bukkit.event.player.PlayerInteractEntityEvent e){if(plugin.displays().owns(e.getRightClicked()))e.setCancelled(true);}
    ...
    @EventHandler public void join(PlayerJoinEvent e) {
        ...
        long grace=plugin.getConfig().getLong("eligibility.recently-joined-grace-seconds",0);if(grace>0)Bukkit.getScheduler().runTaskLater(plugin,()->assignPending(e.getPlayer()),grace*20+1);
    }
    private void assignPending(Player player){if(!player.isOnline())return;for(DragonSoul soul:souls.all())if(soul.state()==DragonSoulState.TRANSFER_PENDING&&!scheduledTransfers.contains(soul.id())&&eligibility.check(player).eligible()){souls.assign(soul.id(),player.getUniqueId(),"JOIN_PENDING");dragonborn.apply(player);plugin.animations().play("soul-arrive",player.getLocation(),player);Bukkit.broadcast(plugin.messages().component("reincarnation","player",player.getName(),"soul",SoulIdentity.displayName(soul.id())));break;}}
    @EventHandler public void respawn(PlayerRespawnEvent e) { Bukkit.getScheduler().runTaskLater(plugin, () -> dragonborn.apply(e.getPlayer()), 1); }
    @EventHandler public void worldChange(PlayerChangedWorldEvent e) { Bukkit.getScheduler().runTask(plugin,()->dragonborn.apply(e.getPlayer())); }
    @EventHandler public void dragonbornGain(DragonbornGainEvent e){plugin.players().recordHistory(e.player(),"GAIN",e.soulId());Player player=Bukkit.getPlayer(e.player());if(player!=null){dragonborn.apply(player);abilities.setEnergy(player,abilities.maxEnergy());abilities.select(player,"wings");}}
    @EventHandler public void dragonbornLose(DragonbornLoseEvent e){plugin.players().recordHistory(e.player(),"LOSE",e.soulId());Player player=Bukkit.getPlayer(e.player());if(player!=null){abilities.resetCombatState(player);dragonborn.remove(player);}}
    @EventHandler(ignoreCancelled=true) public void ritualMove(PlayerMoveEvent e){plugin.rituals().active().filter(r->r.playerId().equals(e.getPlayer().getUniqueId())).ifPresent(r->{if(plugin.configService().file("ritual.yml").getBoolean("restrict-movement",true)&&e.hasChangedPosition()){Location from=e.getFrom(),to=e.getTo();e.setTo(new Location(from.getWorld(),from.getX(),from.getY(),from.getZ(),to.getYaw(),to.getPitch()));}});}
    @EventHandler public void playerDeath(PlayerDeathEvent e) {
        Player victim=e.getPlayer();abilities.resetCombatState(victim);e.getDrops().removeIf(dragonborn::isFocus);Optional<DragonSoul> held=souls.byHolder(victim.getUniqueId()); if (held.isEmpty()) return;
        DragonSoul soul=held.get(); Player killer=victim.getKiller();
        if (killer != null && !dragonborn.isDragonborn(killer.getUniqueId()) && eligibility.check(killer).eligible()) {
            souls.assign(soul.id(), killer.getUniqueId(), "PVP_INHERITANCE"); dragonborn.remove(victim); dragonborn.apply(killer);
            plugin.animations().play("soul-depart",victim.getLocation(),victim);plugin.animations().play("pvp-transfer",killer.getLocation(),killer);
            Bukkit.broadcast(plugin.messages().component("dragonborn-transfer","from",victim.getName(),"soul",SoulIdentity.displayName(soul.id()),"to",killer.getName()));
    ...
    private void scheduleReincarnation(DragonSoul soul){
        if(!scheduledTransfers.add(soul.id()))return;
        int countdown=plugin.getConfig().getInt("transfer.natural-death-countdown-seconds",10);Bukkit.broadcast(plugin.messages().component("natural-death","seconds",Integer.toString(countdown)));
        for(int remaining=countdown;remaining>0;remaining--){int value=remaining;Bukkit.getScheduler().runTaskLater(plugin,()->Bukkit.broadcast(plugin.messages().component("natural-countdown","seconds",Integer.toString(value))),(countdown-remaining)*20L);}
        Bukkit.getScheduler().runTaskLater(plugin,()->{scheduledTransfers.remove(soul.id());DragonSoul current=souls.byId(soul.id()).orElse(null);if(current==null||current.state()!=DragonSoulState.TRANSFER_PENDING)return;List<Player> candidates=eligibility.eligible(Bukkit.getOnlinePlayers());if(candidates.isEmpty()){Bukkit.broadcast(plugin.messages().component("no-eligible-recipient"));return;}Player chosen=candidates.get(new Random().nextInt(candidates.size()));souls.assign(soul.id(),chosen.getUniqueId(),"NATURAL_REINCARNATION");dragonborn.apply(chosen);plugin.animations().play("soul-arrive",chosen.getLocation(),chosen);plugin.animations().play("natural-transfer",chosen.getLocation(),chosen);Bukkit.broadcast(plugin.messages().component("reincarnation","player",chosen.getName(),"soul",SoulIdentity.displayName(soul.id())));},countdown*20L);
    }
    @EventHandler(priority=EventPriority.HIGHEST,ignoreCancelled=true) public void combat(EntityDamageByEntityEvent e){
        ...
        if(e.getEntity() instanceof Player victim){Player attacker=null;if(e.getDamager() instanceof Player p)attacker=p;else if(e.getDamager() instanceof Projectile projectile&&projectile.getShooter() instanceof Player p)attacker=p;if(attacker!=null&&attacker!=victim)combatTags.tag(attacker,victim);}
    }
    @EventHandler(priority=EventPriority.MONITOR,ignoreCancelled=true) public void bulwarkPreventedDamage(EntityDamageEvent e){
        abilities.handleBulwarkDamage(e);
    }
    @EventHandler(priority=EventPriority.HIGHEST,ignoreCancelled=true) public void resonanceDefense(EntityDamageEvent e){
        abilities.handleResonanceDefense(e);
    }
    @EventHandler(ignoreCancelled=true) public void passiveDamage(EntityDamageEvent e){
        if(e.getEntity() instanceof Player ritualPlayer&&plugin.rituals().active().filter(r->r.playerId().equals(ritualPlayer.getUniqueId())).isPresent()&&plugin.configService().file("ritual.yml").getBoolean("failure.cancel-on-damage",true)){plugin.rituals().cancel(plugin.configService().file("ritual.yml").getBoolean("refund-on-cancel",true));return;}
        if(!(e.getEntity() instanceof Player p)||!dragonborn.isDragonborn(p.getUniqueId()))return;
        if(dragonborn.hasSoul(p,SoulIdentity.AKUMA)&&e.getCause()==EntityDamageEvent.DamageCause.FREEZE){e.setCancelled(true);p.setFreezeTicks(0);}
        if(dragonborn.hasSoul(p,SoulIdentity.REV)&&EnumSet.of(EntityDamageEvent.DamageCause.FIRE,EntityDamageEvent.DamageCause.FIRE_TICK,EntityDamageEvent.DamageCause.LAVA,EntityDamageEvent.DamageCause.HOT_FLOOR).contains(e.getCause()))e.setCancelled(true);
        if(dragonborn.hasSoul(p,SoulIdentity.LAMARI)&&e.getCause()==EntityDamageEvent.DamageCause.FALL)e.setCancelled(true);
    }
    @EventHandler(ignoreCancelled=true) public void endermanTarget(EntityTargetLivingEntityEvent e){
        if(e.getEntityType()==EntityType.ENDERMAN&&e.getTarget() instanceof Player p&&dragonborn.isDragonborn(p.getUniqueId())&&plugin.configService().file("abilities.yml").getBoolean("passives.neutral-endermen",true))e.setCancelled(true);
    }
    @EventHandler public void quit(PlayerQuitEvent e){
        Player victim=e.getPlayer();abilities.handleLogout(victim);if(!combatTags.tagged(victim.getUniqueId()))return;Optional<DragonSoul> soul=souls.byHolder(victim.getUniqueId());if(soul.isEmpty())return;
        combatTags.opponent(victim.getUniqueId()).map(Bukkit::getPlayer).filter(Objects::nonNull).filter(p->!dragonborn.isDragonborn(p.getUniqueId())&&eligibility.check(p).eligible()).ifPresentOrElse(killer->{souls.assign(soul.get().id(),killer.getUniqueId(),"COMBAT_LOG");dragonborn.remove(victim);dragonborn.apply(killer);plugin.animations().play("pvp-transfer",killer.getLocation(),killer);Bukkit.broadcast(plugin.messages().component("combat-log-transfer","player",victim.getName(),"soul",SoulIdentity.displayName(soul.get().id()),"recipient",killer.getName()));},()->souls.pending(soul.get().id(),"COMBAT_LOG_NO_RECIPIENT"));combatTags.clear(victim.getUniqueId());
    }
