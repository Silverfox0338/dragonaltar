...
public final class AbilityService {
    ...
    public void start() {
        ...
        task = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            ...
            for (Player p : Bukkit.getOnlinePlayers()) if (dragonborn.isDragonborn(p.getUniqueId())) {
                ...
                int regen=baseRegen;
                if(dragonborn.hasSoul(p,SoulIdentity.REV)&&(p.getWorld().getEnvironment()==World.Environment.NETHER||p.getWorld().getEnvironment()==World.Environment.THE_END))
                    regen=(int)Math.ceil(regen*config.file("abilities.yml").getDouble("named-souls.rev.dimension-energy-multiplier",1.5));
                ...
                if(players.settings(p.getUniqueId()).hud())p.sendActionBar(plugin.messages().component("energy-hud",
                        "energy",Integer.toString(current(p)),"maximum",Integer.toString(max),"ability",abilityName,
                        "cooldown",formatCooldown(cooldown),
                        "ultimate_cooldown",formatCooldown(ultimateCooldown),
                        "resonance",resonance.map(DragonResonance::displayName).orElse("Locked"),
                        "resonance_cooldown",resonance.isEmpty()?"Locked":formatCooldown(resonanceCooldown),
                        "status",combatStatus(p)));
                if(config.file("abilities.yml").getBoolean("passives.particles",true)&&players.settings(p.getUniqueId()).passiveParticles()&&players.settings(p.getUniqueId()).effects()!=com.dragonaltar.player.EffectMode.MINIMAL)
                    passiveParticleAccessible(p.getLocation().add(0,1,0),Particle.DRAGON_BREATH,
                            players.settings(p.getUniqueId()).effects()==com.dragonaltar.player.EffectMode.FULL?3:1,.35,.6,.35,0);
            }
        }, interval, interval);
    }
    public void stop() {
    ...
    public void cycleCategory(Player p){
        DragonAbility current=registry.get(selected(p));AbilityCategory[] categories=AbilityCategory.values();
        for(int offset=1;offset<=categories.length;offset++){
            ...
            if(candidate.isPresent()){select(p,candidate.get().id());return;}
        }
    }
    public AbilityResult cast(Player p) {
        if(!p.hasPermission("dragonaltar.use")||!dragonborn.isDragonborn(p.getUniqueId()))return AbilityResult.fail("ability-not-dragonborn");
        if(!p.isOnline()||p.isDead()||p.getGameMode()==GameMode.SPECTATOR)return AbilityResult.fail("ability-unavailable");
        DragonAbility ability = registry.get(selected(p)); if (ability == null || !supports(p,ability)) return AbilityResult.fail("Unknown ability");
        ...
        for(double step=0;step<=distance;step+=.75){
            ...
            for(LivingEntity living:nearbyLiving(point,1.7,player)){
                int amplifier=brittle.active(living.getUniqueId(),System.currentTimeMillis())
                        ?integer("akumas-trail.brittle-slowness-amplifier",2):1;
    ...
    private boolean hasNearbyWater(Location center,int radius){
        ...
        for(int x=-radius;x<=radius;x++)for(int y=-2;y<=2;y++)for(int z=-radius;z<=radius;z++)
            if(x*x+z*z<=radius*radius&&origin.getRelative(x,y,z).getType()==Material.WATER)return true;
        return false;
    }
    ...
    private void armWaterRun(Player player){
        ...
        if(players.settings(player.getUniqueId()).hud())player.sendActionBar(mini.deserialize("<aqua>Touch the water within 3 seconds to begin Frost Run!</aqua>"));
        new BukkitRunnable(){int ticks;public void run(){
            if(!player.isOnline()||player.isDead()||ticks++>=entryTicks){cancel();return;}
            particleAccessible(player.getLocation().add(0,.2,0),Particle.SNOWFLAKE,3,.25,.1,.25,.01);
            if(touchesWater(player)){startWaterRun(player);cancel();}
    ...
    private void startWaterRun(Player player){
        ...
        player.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,durationTicks,integer("akumas-trail.water-run-speed-amplifier",2),false,true,true));
    ...
    private void akumasHush(Player player){
        ...
        new BukkitRunnable(){int elapsed;public void run(){
            if(elapsed>=duration||center.getWorld()==null){cancel();return;}
            particleAccessible(center.clone().add(0,.5,0),Particle.SNOWFLAKE,28,radius*.55,.5,radius*.55,.02);
            long now=System.currentTimeMillis();
            for(LivingEntity living:nearbyLiving(center,radius,player)){
                ...
                if(brittle.active(living.getUniqueId(),now))brittle.extend(living.getUniqueId(),now,
    ...
    private void revsRend(Player player){
        ...
        new BukkitRunnable(){int ticks;Location previous=start;boolean recastAnnounced;public void run(){
            if(!player.isOnline()||player.isDead()||!dragonborn.hasSoul(player,SoulIdentity.REV)||ticks++>=maximumTicks
                    ||!player.getWorld().equals(start.getWorld())
                    ||player.getLocation().distanceSquared(start)>maximumDistance*maximumDistance){cancel();return;}
            ...
            for(LivingEntity living:nearbyLiving(current,crossRadius,player)){
                if(crossed.contains(living.getUniqueId())||!validRevPrey(player,living)
                        ||distanceSquaredToSegment(living.getLocation().toVector(),previous.toVector(),current.toVector())
                        >crossRadius*crossRadius)continue;
                ...
                if(!recastAnnounced){
                    long expiry=System.currentTimeMillis()+Math.max(1,Math.min(100,
                            integer("revs-rend.recast-window-ticks",36)))*50L;
                    revHunt.armRecast(player.getUniqueId(),expiry);
                    recastAnnounced=true;
                    if(players.settings(player.getUniqueId()).hud())
                        player.sendActionBar(mini.deserialize("<gold>Rend Recast Ready</gold> <gray>Aim at Inferno Marked prey</gray>"));
                    playConfiguredSound(player.getLocation(),"revs-rend.sounds.recast-ready",Sound.ITEM_FIRECHARGE_USE,.75f,1.3f);
                }
            }
            previous=current;
    ...
    private AbilityResult revsRendRecast(Player player){
        long now=System.currentTimeMillis();
        LivingEntity target=aimedMarkedTarget(player,Math.max(1,Math.min(24,decimal("revs-rend.recast-range",14))),
                decimal("revs-rend.recast-aim-cone-degrees",24),now);
        if(target==null){
            if(players.settings(player.getUniqueId()).hud())
                player.sendActionBar(mini.deserialize("<red>Rend recast failed:</red> <gray>Aim at visible Inferno Marked prey in range</gray>"));
            playConfiguredSound(player.getLocation(),"revs-rend.sounds.recast-failed",Sound.BLOCK_NOTE_BLOCK_BASS,.65f,.7f);
        ...
        player.setVelocity(toward.normalize().multiply(Math.min(Math.min(2,decimal("revs-rend.recast-surge-strength",1.45)),
        ...
        if(claim){
            ...
            playConfiguredSound(target.getLocation(),"revs-rend.sounds.predators-claim",Sound.ENTITY_ENDER_DRAGON_GROWL,1f,1.45f);
    ...
    private void wrathOfRev(Player player){
        ...
        new BukkitRunnable(){int pulse;public void run(){
            if(!player.isOnline()||player.isDead()||!dragonborn.hasSoul(player,SoulIdentity.REV)||pulse>=pulses){cancel();return;}
            pulse++;
            Location center=player.getLocation().clone();
            ...
            playConfiguredSound(player.getLocation(),"wrath-of-rev.sounds.pulse",Sound.ENTITY_BLAZE_SHOOT,.7f,.8f+(pulse*.04f));
            long now=System.currentTimeMillis();
            for(LivingEntity living:nearbyLiving(player.getLocation(),radius,player)){
                if(!validRevPrey(player,living)||!player.hasLineOfSight(living))continue;
                boolean firstContact=encountered.add(living.getUniqueId());
                if(firstContact)applyInfernoMark(player,living,now);
                ...
                living.setFireTicks(Math.max(living.getFireTicks(),integer("wrath-of-rev.pressure-fire-ticks",30)));
                showMarkedTrailToRev(player,living);
            }
            if(pulse==pulses&&revHunt.heat(player.getUniqueId())>=integer("wrath-of-rev.pursuit-surge-heat-threshold",60))
                wrathPursuitSurge(player,now);
        }}.runTaskTimer(plugin,0,interval);
    }

    private void wrathPursuitSurge(Player player,long now){
        LivingEntity nearest=nearbyLiving(player.getLocation(),Math.max(1,Math.min(24,decimal("wrath-of-rev.pursuit-surge-range",11))),player).stream()
                .filter(target->validRevPrey(player,target)&&revHunt.markedBy(target.getUniqueId(),player.getUniqueId(),now)
                        &&player.hasLineOfSight(target)&&clearPath(player,target))
                .min(Comparator.comparingDouble(target->target.getLocation().distanceSquared(player.getLocation()))).orElse(null);
        if(nearest!=null){
            Vector toward=nearest.getLocation().toVector().subtract(player.getLocation().toVector());
            if(toward.lengthSquared()>.01)player.setVelocity(toward.normalize().multiply(
                    Math.min(1,decimal("wrath-of-rev.pursuit-surge-strength",.65))).setY(
                            Math.min(.4,decimal("wrath-of-rev.pursuit-surge-lift",.12))));
        }else player.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,
                integer("wrath-of-rev.fallback-speed-ticks",30),
                integer("wrath-of-rev.fallback-speed-amplifier",1),false,true,true));
        playConfiguredSound(player.getLocation(),"wrath-of-rev.sounds.pursuit-surge",Sound.ENTITY_BLAZE_SHOOT,.9f,1.4f);
    }
    private void lamarisFault(Player player){
        player.setFlying(false);
        player.setVelocity(new Vector(0,-decimal("lamaris-fault.slam-speed",2.4),0));
        new BukkitRunnable(){int ticks;public void run(){
            if(!player.isOnline()||ticks++>80){cancel();return;}
            particleAccessible(player.getLocation(),Particle.BLOCK,5,.3,.3,.3,.02,Material.DEEPSLATE.createBlockData());
            if(ticks>2&&isGrounded(player)){earthPulse(player,decimal("lamaris-fault.radius",7),false);cancel();}
        }}.runTaskTimer(plugin,1,1);
    }
    private void lamarisReckoning(Player player){earthPulse(player,decimal("lamaris-reckoning.radius",10),true);}
    private void earthPulse(Player player,double radius,boolean knockup){
        Location center=player.getLocation();
        particleAccessible(center,Particle.BLOCK,90,radius*.5,.35,radius*.5,.08,Material.DEEPSLATE.createBlockData());
        ...
        for(LivingEntity living:nearbyLiving(center,radius,player)){
            ...
            if(!knockup)living.addPotionEffect(new PotionEffect(PotionEffectType.MINING_FATIGUE,integer("lamaris-fault.fatigue-seconds",8)*20,1));
        }
    }
    ...
    private void absoluteZeroImpact(Player player,Location center){
        if(!player.isOnline()||player.isDead()||!dragonborn.hasSoul(player,SoulIdentity.AKUMA)||center.getWorld()==null)return;
        ...
        List<LivingEntity> targets=nearbyLiving(center,radius,player).stream().filter(player::hasLineOfSight).toList();
        ...
        for(LivingEntity target:targets){
            ...
            try{target.damage(decimal("absolute-zero.burst-damage",14),player);}
            finally{suppressCombatInteractions=false;}
            if(!target.isValid()||target.isDead())continue;
            brittle.apply(target.getUniqueId(),player.getUniqueId(),brittleExpiry);
            target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS,trapTicks,integer("absolute-zero.slowness-amplifier",4),false,true,true));
            target.addPotionEffect(new PotionEffect(PotionEffectType.MINING_FATIGUE,trapTicks,integer("absolute-zero.mining-fatigue-amplifier",3),false,true,true));
            if(target instanceof Player affected){
                if(players.settings(affected.getUniqueId()).screenEffects())
                    affected.setFreezeTicks(Math.max(affected.getFreezeTicks(),integer("absolute-zero.screen-freeze-ticks",60)));
                if(players.settings(affected.getUniqueId()).hud())
                    affected.sendActionBar(mini.deserialize("<aqua>Brittle</aqua> <gray>Your next hit will shatter the frost</gray>"));
            }
        ...
        new BukkitRunnable(){int pulse;public void run(){
            if(pulse++>=dotSeconds){
                ...
                particleAccessible(center.clone().add(0,1,0),Particle.CLOUD,
                        integer("absolute-zero.presentation.ending-particles",24),radius*.35,.5,radius*.35,.01);
                applyUltimateDownside(player,"absolute-zero");
                cancel();return;
            }
            for(LivingEntity target:targets)if(target.isValid()&&!target.isDead()){
                suppressCombatInteractions=true;
                try{target.damage(decimal("absolute-zero.frost-damage",2),player);}
                finally{suppressCombatInteractions=false;}
    ...
    private Location iceShellLocation(LivingEntity target,int index,int count,double rotation,double radius,double scale){
    ...
    private void infernosWrathImpact(Player player,Location center){
        if(!player.isOnline()||player.isDead()||!dragonborn.hasSoul(player,SoulIdentity.REV)||center.getWorld()==null)return;
        ...
        int maximumSpeedTicks=Math.min(huntTicks,Math.max(speedTicks,integer("infernos-wrath.maximum-mobility-ticks",140)));
        long now=System.currentTimeMillis();
        ...
        for(LivingEntity target:targets){
            if(!validRevPrey(player,target))continue;
        ...
        new BukkitRunnable(){int ticks;public void run(){
            ...
            if(ticks>=huntTicks){
                playConfiguredSound(player.getLocation(),"infernos-wrath.sounds.exhaustion",Sound.ENTITY_BLAZE_DEATH,1f,.8f);
                ...
                if(players.settings(player.getUniqueId()).hud())
                    player.sendActionBar(mini.deserialize("<dark_red>Hunt exhausted</dark_red> <gray>Unused Predator's Claim expired</gray>"));
                revHunt.endHunt(player.getUniqueId());
                applyUltimateDownside(player,"infernos-wrath");
                cancel();return;
            }
            double ringRadius=radius*Math.min(1,ticks/(double)Math.max(1,integer("infernos-wrath.presentation.ring-expand-ticks",30)));
            ...
            for(int point=0;point<points;point++){
                ...
                particleAccessible(center.clone().add(Math.cos(angle)*ringRadius,.2,Math.sin(angle)*ringRadius),
    ...
    private void titansBulwark(Player player){
        ...
        new BukkitRunnable(){int ticks;public void run(){
            ...
            if(ticks%5==0){
                ...
                if(ticks%20==0)playAccessibleSound(player.getLocation(),Sound.BLOCK_STONE_HIT,.35f,(float)(.65+ratio*.7));
            }
        }}.runTaskTimer(plugin,0,1);
    }
    ...
    private void bulwarkExpiry(Player player,double stored){
        ...
        double knockback=AbilityCombatRules.scaledBulwarkValue(stored,cap,
                decimal("titans-bulwark.expiry-min-knockback",.8),decimal("titans-bulwark.expiry-max-knockback",1.5));
        playAccessibleSound(center,Sound.ENTITY_IRON_GOLEM_DAMAGE,1.25f,(float)(.65+Math.min(1,stored/Math.max(1,cap))*.35));
        ...
        for(LivingEntity target:nearbyLiving(center,radius,player).stream().filter(player::hasLineOfSight).toList()){
            ...
            suppressCombatInteractions=true;
            try{target.damage(damage,player);}
            finally{suppressCombatInteractions=false;}
        }
    ...
    private void activateResonance(Player caster,DragonResonance resonance){
        ...
        for(Player member:team){
            ...
            plugin.messages().send(member,"resonance-cast","ability",resonance.displayName(),"caster",caster.getName());
        }
        switch(resonance){
    ...
    private void thermalConvergence(Player caster,List<Player> team,Location center){
        ...
        List<AnchoredDisplay> armor=spawnTeamOrbit(team,new Material[]{Material.BLUE_ICE,Material.MAGMA_BLOCK},
                resonanceInteger("thermal-convergence.displays-per-player",3),resonanceDecimal("thermal-convergence.display-scale",.34));
        ...
        new BukkitRunnable(){int ticks;public void run(){
            if(!validTeam(team)||ticks>impactDelay+activeTicks){
                removeAnchoredDisplays(armor);cancel();return;
            }
            ...
            if(ticks%5==0){
                particleLink(team.get(0).getLocation().add(0,1,0),team.get(1).getLocation().add(0,1,0),
                        ticks%10==0?Particle.SNOWFLAKE:Particle.FLAME,resonanceInteger("thermal-convergence.link-points",14));
                particleAccessible(midpoint.clone().add(0,1,0),ticks%10==0?Particle.SNOWFLAKE:Particle.FLAME,
                        resonanceInteger("thermal-convergence.active-particles",12),1.4,.8,1.4,.03);
            }
            if(ticks==impactDelay)thermalImpact(caster,team,midpoint);
            int active=ticks-impactDelay;
            if(active>0&&active<activeTicks&&active%pulseInterval==0)thermalPulse(caster,team,midpoint,active/pulseInterval);
    ...
    private void volcanicAegis(Player caster,List<Player> team,Location center){
        ...
        List<AnchoredDisplay> armor=spawnTeamOrbit(team,new Material[]{Material.MAGMA_BLOCK,Material.POLISHED_BLACKSTONE},
                resonanceInteger("volcanic-aegis.displays-per-player",5),resonanceDecimal("volcanic-aegis.display-scale",.42));
        ...
        new BukkitRunnable(){int ticks;public void run(){
            if(!validTeam(team)||ticks>impactDelay+activeTicks){
                clearVolcanicAegis(team);removeAnchoredDisplays(armor);cancel();return;
            }
            updateTeamOrbit(armor,ticks,resonanceDecimal("volcanic-aegis.display-orbit-radius",.72),.1);
            if(ticks%5==0)for(Player member:team)
            ...
            if(ticks==impactDelay)volcanicImpact(caster,team);
            int active=ticks-impactDelay;
            if(active>0&&active<activeTicks&&active%pulseInterval==0)volcanicPulse(caster,team);
            if(active==activeTicks){
    ...
    private void glacialBastion(Player caster,List<Player> team,Location center){
        ...
        List<AnchoredDisplay> armor=spawnTeamOrbit(team,new Material[]{Material.BLUE_ICE,Material.AMETHYST_BLOCK},
                resonanceInteger("glacial-bastion.displays-per-player",5),resonanceDecimal("glacial-bastion.display-scale",.4));
        ...
        new BukkitRunnable(){int ticks;public void run(){
            if(!validTeam(team)||ticks>impactDelay+activeTicks){
                clearGlacialWards(team);removeAnchoredDisplays(armor);cancel();return;
            }
            ...
            if(ticks==impactDelay)glacialImpact(team);
            int active=ticks-impactDelay;
            if(active>0&&active<activeTicks&&active%pulseInterval==0)glacialPulse(caster,team);
            if(active==activeTicks){
    ...
    private void dragonTrinity(Player caster,List<Player> team,Location center){
        ...
        List<BlockDisplay> sigil=spawnCenterOrbit(center,new Material[]{Material.BLUE_ICE,Material.MAGMA_BLOCK,Material.DEEPSLATE_TILES},
                resonanceInteger("dragon-trinity.sigil-displays",9),resonanceDecimal("dragon-trinity.display-scale",.5));
        ...
        new BukkitRunnable(){int ticks;public void run(){
            ...
            if(ticks%4==0){
                for(Player member:team)particleLink(member.getLocation().add(0,1.2,0),midpoint.clone().add(0,1.3,0),
            ...
            if(ticks==impactDelay)trinityImpact(caster,team,midpoint);
            int active=ticks-impactDelay;
    ...
    private void thermalImpact(Player caster,List<Player> team,Location center){
        ...
        for(Player member:team){
            member.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,duration,resonanceInteger("thermal-convergence.speed-amplifier",1)));
        ...
        particleAccessible(center.clone().add(0,1,0),Particle.FLAME,resonanceInteger("thermal-convergence.impact-particles",88),radius*.45,1.2,radius*.45,.09);
        particleAccessible(center.clone().add(0,1,0),Particle.SNOWFLAKE,resonanceInteger("thermal-convergence.impact-particles",88)/2,radius*.4,1,radius*.4,.06);
    ...
    private void thermalPulse(Player caster,List<Player> team,Location center,int pulse){
        ...
        playAccessibleSound(center,pulse%2==0?Sound.BLOCK_GLASS_HIT:Sound.ENTITY_BLAZE_SHOOT,.7f,pulse%2==0?1.45f:.95f);
        particleAccessible(center.clone().add(0,1,0),particle,resonanceInteger("thermal-convergence.pulse-particles",36),radius*.35,.8,radius*.35,.05);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
    ...
    private void thermalFinisher(Player caster,List<Player> team,Location center){
        ...
        particleAccessible(center.clone().add(0,1,0),Particle.FLASH,resonanceInteger("thermal-convergence.flash-particles",2),0,0,0,0);
        particleAccessible(center.clone().add(0,1,0),Particle.ITEM_SNOWBALL,resonanceInteger("thermal-convergence.finisher-particles",64),radius*.45,1.1,radius*.45,.15);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
    ...
    private void applyThermalDebuffs(LivingEntity target){
        ...
        target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS,seconds*20,resonanceInteger("thermal-convergence.slowness-amplifier",2)));
        target.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS,seconds*20,resonanceInteger("thermal-convergence.weakness-amplifier",1)));
        target.setFireTicks(Math.max(target.getFireTicks(),resonanceInteger("thermal-convergence.fire-seconds",3)*20));
        if(!(target instanceof Player affected)||players.settings(affected.getUniqueId()).screenEffects())
            target.setFreezeTicks(Math.max(target.getFreezeTicks(),resonanceInteger("thermal-convergence.freeze-ticks",50)));
    }
    ...
    private void volcanicImpact(Player caster,List<Player> team){
        ...
        for(Player member:team){
            ...
            particleAccessible(member.getLocation().add(0,1,0),Particle.LAVA,resonanceInteger("volcanic-aegis.impact-particles",64),1,.9,1,.04);
        }
        for(LivingEntity target:resonanceTargetsAroundTeam(team,resonanceDecimal("volcanic-aegis.pulse-radius",6))){
            dealAbilityDamage(target,resonanceDecimal("volcanic-aegis.initial-damage",10),caster);
            target.setFireTicks(Math.max(target.getFireTicks(),resonanceInteger("volcanic-aegis.fire-seconds",6)*20));
        }
    }
    ...
    private void volcanicPulse(Player caster,List<Player> team){
        for(Player member:team){
            ...
            particleAccessible(member.getLocation().add(0,.8,0),Particle.FLAME,resonanceInteger("volcanic-aegis.pulse-particles",30),
                    resonanceDecimal("volcanic-aegis.pulse-radius",6)*.35,.6,resonanceDecimal("volcanic-aegis.pulse-radius",6)*.35,.04);
        }
        for(LivingEntity target:resonanceTargetsAroundTeam(team,resonanceDecimal("volcanic-aegis.pulse-radius",6))){
    ...
    private void volcanicFinisher(Player caster,List<Player> team,Location center){
        ...
        particleAccessible(center.clone().add(0,1,0),Particle.LAVA,resonanceInteger("volcanic-aegis.finisher-particles",72),radius*.45,1,radius*.45,.1);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
    ...
    private void glacialImpact(List<Player> team){
        ...
        for(Player member:team){
            ...
            particleAccessible(member.getLocation().add(0,1,0),Particle.SNOWFLAKE,resonanceInteger("glacial-bastion.impact-particles",72),1,.9,1,.05);
        }
    }

    private void glacialPulse(Player caster,List<Player> team){
        double radius=resonanceDecimal("glacial-bastion.domain-radius",7);
        for(Player member:team)particleAccessible(member.getLocation().add(0,.5,0),Particle.SNOWFLAKE,
        ...
        for(LivingEntity target:resonanceTargetsAroundTeam(team,radius)){
            ...
            target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS,seconds*20,resonanceInteger("glacial-bastion.slowness-amplifier",3)));
            target.addPotionEffect(new PotionEffect(PotionEffectType.MINING_FATIGUE,seconds*20,resonanceInteger("glacial-bastion.mining-fatigue-amplifier",2)));
            if(!(target instanceof Player affected)||players.settings(affected.getUniqueId()).screenEffects())
                target.setFreezeTicks(Math.max(target.getFreezeTicks(),resonanceInteger("glacial-bastion.freeze-ticks",80)));
        }
    }
    ...
    private void glacialFinisher(Player caster,List<Player> team,Location center){
        ...
        particleAccessible(center.clone().add(0,1,0),Particle.ITEM_SNOWBALL,resonanceInteger("glacial-bastion.finisher-particles",80),radius*.45,1,radius*.45,.14);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
    ...
    private void trinityImpact(Player caster,List<Player> team,Location center){
        ...
        for(Player member:team){
            member.addPotionEffect(new PotionEffect(PotionEffectType.STRENGTH,duration,resonanceInteger("dragon-trinity.strength-amplifier",1)));
            member.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,duration,resonanceInteger("dragon-trinity.speed-amplifier",1)));
            member.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE,duration,resonanceInteger("dragon-trinity.resistance-amplifier",1)));
            member.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION,duration,resonanceInteger("dragon-trinity.absorption-amplifier",3)));
        }
        ...
        particleAccessible(center.clone().add(0,1,0),Particle.DRAGON_BREATH,resonanceInteger("dragon-trinity.impact-particles",128),radius*.5,1.5,radius*.5,.12);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
            ...
            target.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING,resonanceInteger("dragon-trinity.reveal-seconds",12)*20,0));
        }
    }
    ...
    private void trinityPulse(Player caster,List<Player> team,Location center,int pulse){
        ...
        for(Player member:team){
            ...
            if(health!=null)member.setHealth(Math.min(health.getValue(),member.getHealth()+resonanceDecimal("dragon-trinity.healing-per-pulse",2)));
            setEnergy(member,Math.min(maxEnergy(),current(member)+resonanceInteger("dragon-trinity.energy-per-pulse",5)));
        }
    ...
    private void trinityFinisher(Player caster,List<Player> team,Location center){
        ...
        particleAccessible(center.clone().add(0,1.5,0),Particle.FLASH,resonanceInteger("dragon-trinity.flash-particles",3),0,0,0,0);
        particleAccessible(center.clone().add(0,1,0),Particle.END_ROD,resonanceInteger("dragon-trinity.finisher-particles",128),radius*.5,1.8,radius*.5,.16);
        for(LivingEntity target:resonanceTargets(center,radius,team)){
    ...
    private List<LivingEntity> resonanceTargetsAroundTeam(List<Player> team,double radius){
        ...
        for(Player member:team)if(member.isOnline())for(LivingEntity target:member.getWorld().getNearbyLivingEntities(member.getLocation(),boundedRadius,
                living->validTarget(living,null)&&!friendly.contains(living.getUniqueId())&&member.hasLineOfSight(living)))
    ...
    private void pullToward(LivingEntity target,Location center,double strength){
        ...
        if(toward.lengthSquared()<.01)return;
    ...
    private void resonancePhase(List<Player> team,String message){
        ...
        for(Player member:team)if(member.isOnline()&&players.settings(member.getUniqueId()).hud())member.sendActionBar(component);
    }

    private List<AnchoredDisplay> spawnTeamOrbit(List<Player> team,Material[] materials,int requestedPerPlayer,double scale){
        int count=Math.max(2,Math.min(8,requestedPerPlayer));
        List<AnchoredDisplay> displays=new ArrayList<>();
        for(Player member:team)for(int index=0;index<count;index++){
            BlockDisplay display=spawnAbilityBlock(member.getLocation().add(0,1,0),materials[index%materials.length],scale);
    ...
    private void updateTeamOrbit(List<AnchoredDisplay> displays,int ticks,double radius,double speed){
        for(AnchoredDisplay anchored:displays){
            if(!anchored.anchor().isOnline()||!anchored.display().isValid())continue;
    ...
    private List<BlockDisplay> spawnCenterOrbit(Location center,Material[] materials,int requested,double scale){
        ...
        for(int index=0;index<count;index++)displays.add(spawnAbilityBlock(center.clone().add(0,1,0),materials[index%materials.length],scale));
        return displays;
    }

    private void updateCenterOrbit(List<BlockDisplay> displays,Location center,int ticks,double radius,double speed){
        for(int index=0;index<displays.size();index++){
            BlockDisplay display=displays.get(index);if(!display.isValid())continue;
    ...
    private BlockDisplay spawnAbilityBlock(Location location,Material material,double scale){
        ...
        BlockDisplay display=location.getWorld().spawn(location,BlockDisplay.class,entity->{
            entity.setBlock(material.createBlockData());entity.setPersistent(false);entity.setViewRange(40);
    ...
    private void particleLink(Location from,Location to,Particle particle,int requestedPoints){
        ...
        for(int point=0;point<=points;point++)
            particleAccessible(from.clone().add(path.clone().multiply(point/(double)points)),particle,1,0,0,0,.01);
    }

    private boolean validTeam(List<Player> team){
        if(team.isEmpty())return false;
        World world=team.getFirst().getWorld();
        return team.stream().allMatch(member->member.isOnline()&&!member.isDead()&&member.getWorld().equals(world));
    }
    ...
    private List<LivingEntity> resonanceTargets(Location center,double radius,List<Player> team){
        Set<UUID> friendly=team.stream().map(Player::getUniqueId).collect(java.util.stream.Collectors.toSet());
        double boundedRadius=boundedTargetRadius(radius);
        return center.getWorld().getNearbyLivingEntities(center,boundedRadius,target->validTarget(target,null)
                &&!friendly.contains(target.getUniqueId())
                &&team.stream().anyMatch(member->member.isOnline()&&member.getWorld().equals(target.getWorld())&&member.hasLineOfSight(target)))
                .stream().limit(MAX_TARGETS_PER_QUERY).toList();
    }

    private void dealAbilityDamage(LivingEntity target,double damage,Player source){
        suppressCombatInteractions=true;
        try{target.damage(Math.max(0,damage),source);}finally{suppressCombatInteractions=false;}
    }
    ...
    private Location teamCenter(List<Player> team){
        World world=team.getFirst().getWorld();
        double x=0,y=0,z=0;
        for(Player member:team){x+=member.getX();y+=member.getY();z+=member.getZ();}
    ...
    private void applyUltimateDownside(Player player,String ability){
        ...
        PotionEffectType primary=ability.equals("titans-bulwark")?PotionEffectType.MINING_FATIGUE:PotionEffectType.SLOWNESS;
        PotionEffectType secondary=PotionEffectType.WEAKNESS;
        if(duration>0){
            if(first>=0)player.addPotionEffect(new PotionEffect(primary,duration*20,first,false,true,true));
    ...
    public void handleResonanceDefense(EntityDamageEvent event){
        ...
        if(players.settings(defender.getUniqueId()).hud())
            defender.sendActionBar(mini.deserialize("<aqua>Crystal Ward absorbed damage</aqua> <gray>| Charges: "+remaining+"</gray>"));
    }
    ...
    public void handleCombatEffects(EntityDamageByEntityEvent event){
        ...
        long now=System.currentTimeMillis();
        Optional<AbilityCombatRules.Brittle> shattered=brittle.consume(target.getUniqueId(),now,event.getFinalDamage());
        ...
        if(attacker!=null&&dragonborn.hasSoul(attacker,SoulIdentity.REV)
                &&validRevPrey(attacker,target)
            ...
            if(AbilityCombatRules.RevHuntTracker.acceptsDamage(source)){
                ...
                AbilityCombatRules.RampageGain rampage=revHunt.gainRampage(attacker.getUniqueId(),target.getUniqueId(),now,
                        integer("infernos-wrath.rampage-progress-per-hit",1),
                ...
                if(rampage.granted()){
                    ...
                    playConfiguredSound(attacker.getLocation(),"infernos-wrath.sounds.rampage-gain",Sound.ENTITY_BLAZE_SHOOT,.65f,1.35f);
                    ...
                    if(rampage.maximumReached()){
                        ...
                        showTitle(attacker,"<red><bold>PREDATOR'S CLAIM READY</bold>","<gold>Consume it with Rend's marked recast</gold>");
                        playConfiguredSound(attacker.getLocation(),"infernos-wrath.sounds.rampage-maximum",Sound.ENTITY_WITHER_SPAWN,.7f,1.35f);
                    }
                }
            }
        }
    ...
    public void handleBulwarkMelee(EntityDamageByEntityEvent event){
        ...
        if(reflected>0&&reflectionGuard.enter()){
            suppressCombatInteractions=true;
            try{attacker.damage(reflected,defender);}
            finally{suppressCombatInteractions=false;reflectionGuard.exit();}
        }
        attacker.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS,integer("titans-bulwark.reflected-slowness-seconds",2)*20,1));
    }
    ...
    private void grantHeat(Player rev,LivingEntity target,int amount,long now){
        ...
        if(previous<mobilityThreshold&&gain.heat()>=mobilityThreshold){
            rev.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,
                    integerRoot("rev-hunt.heat.threshold-speed-ticks",30),
        ...
        if(gain.finisherArmed()&&previous<integerRoot("rev-hunt.heat.maximum",100)){
            revHunt.armFinisher(rev.getUniqueId(),now+integerRoot("rev-hunt.finisher.armed-duration-ticks",120)*50L);
            showTitle(rev,"<red><bold>PREDATOR'S CLAIM READY</bold>","<gold>Cross prey with Rend, then recast</gold>");
            playConfiguredSound(rev.getLocation(),"rev-hunt.sounds.finisher-ready",Sound.ENTITY_WITHER_SPAWN,.65f,1.45f);
        }
    }
    ...
    private boolean validRevPrey(Player rev,LivingEntity target){
        if(target==rev||!target.isValid()||target.isDead()||target instanceof ArmorStand
                ||target instanceof Player player&&player.getGameMode()==GameMode.SPECTATOR)return false;
        if(dragonborn.isDragonborn(target.getUniqueId()))return false;
    ...
    private LivingEntity aimedMarkedTarget(Player player,double range,double coneDegrees,long now){
        ...
        return nearbyLiving(player.getLocation(),range,player).stream()
                ...
                .filter(target->{
                    ...
                    Vector toward=target.getEyeLocation().toVector().subtract(player.getEyeLocation().toVector()).normalize();
    ...
    private void showRevTracking(Player rev,long now){
        if(revHunt.heat(rev.getUniqueId())<integerRoot("rev-hunt.heat.tracking-threshold",65))return;
        var settings=players.settings(rev.getUniqueId());
        if(!settings.animationParticles()||settings.effects()==com.dragonaltar.player.EffectMode.MINIMAL)return;
        ...
        for(LivingEntity target:nearbyLiving(rev.getLocation(),range,rev)){
            ...
            if(validRevPrey(rev,target)&&revHunt.markedBy(target.getUniqueId(),rev.getUniqueId(),now)
                    &&rev.hasLineOfSight(target)){showMarkedTrailToRev(rev,target);shown++;}
        }
    }
    ...
    private void showMarkedTrailToRev(Player rev,LivingEntity target){
        ...
        int count=settings.effects()==com.dragonaltar.player.EffectMode.REDUCED?Math.max(1,requested/2):requested;
        Vector direction=target.getLocation().toVector().subtract(rev.getLocation().toVector());
        if(direction.lengthSquared()<.01)return;
        Location cue=rev.getEyeLocation().add(direction.normalize().multiply(
                decimalRoot("rev-hunt.tracking.directional-cue-distance",1.8)));
    ...
    private void playConfiguredSound(Location location,String path,Sound fallback,float fallbackVolume,float fallbackPitch){
        String configured=config.file("abilities.yml").getString("abilities."+path);
        if(path.startsWith("rev-hunt."))configured=config.file("abilities.yml").getString(path);
        ...
        if(configured!=null){
            ...
            if(matched==null)try{
                ...
                if(legacy instanceof Sound found)matched=found;
            }catch(ReflectiveOperationException ignored){}
            if(matched!=null)sound=matched;
        }
        double volume=path.startsWith("rev-hunt.")?decimalRoot(path+"-volume",fallbackVolume):decimal(path+"-volume",fallbackVolume);
        double pitch=path.startsWith("rev-hunt.")?decimalRoot(path+"-pitch",fallbackPitch):decimal(path+"-pitch",fallbackPitch);
        playAccessibleSound(location,sound,(float)volume,(float)pitch);
    }

    private String combatStatus(Player player){
        long now=System.currentTimeMillis();
        List<String> parts=new ArrayList<>();
        if(brittle.active(player.getUniqueId(),now))parts.add("Brittle");
        if(revHunt.marked(player.getUniqueId(),now))parts.add("Inferno Marked");
        int brittleTargets=brittle.countFor(player.getUniqueId(),now);
        if(brittleTargets>0)parts.add("Brittle targets: "+brittleTargets);
        if(dragonborn.hasSoul(player,SoulIdentity.REV)){
            ...
            if(marks>0)parts.add("Marked: "+marks);
            if(revHunt.recastAvailable(player.getUniqueId(),now))
                parts.add("Rend Recast: "+formatCooldown((long)Math.ceil((revHunt.recastEndsAt(player.getUniqueId())-now)/1000d)));
            if(revHunt.huntActive(player.getUniqueId(),now))
                parts.add("Hunt: "+formatCooldown((long)Math.ceil((revHunt.huntEndsAt(player.getUniqueId())-now)/1000d))
    ...
    private void playAccessibleSound(Location location,Sound sound,float volume,float pitch){
        ...
        for(Player viewer:location.getWorld().getPlayers()){
            if(viewer.getLocation().distanceSquared(location)>viewDistance*viewDistance||!players.settings(viewer.getUniqueId()).sounds())continue;
            viewer.playSound(location,sound,volume,pitch);
        }
    }

    private void particleAccessible(Location location,Particle particle,int requested,
    ...
    private void particleAccessible(Location location,Particle particle,int requested,
        ...
        int viewDistance=integerRoot("presentation.view-distance-blocks",48);
        for(Player viewer:location.getWorld().getPlayers()){
            if(viewer.getLocation().distanceSquared(location)>viewDistance*viewDistance)continue;
            var settings=players.settings(viewer.getUniqueId());
            if(!settings.animationParticles()||settings.effects()==com.dragonaltar.player.EffectMode.MINIMAL)continue;
            int count=settings.effects()==com.dragonaltar.player.EffectMode.REDUCED?Math.max(1,bounded/2):bounded;
            if(count>0){
                if(data==null)viewer.spawnParticle(particle,location,count,offsetX,offsetY,offsetZ,extra);
                else viewer.spawnParticle(particle,location,count,offsetX,offsetY,offsetZ,extra,data);
            }
        }
    }

    private void passiveParticleAccessible(Location location,Particle particle,int requested,
                                           double offsetX,double offsetY,double offsetZ,double extra){
        if(location.getWorld()==null)return;
        int bounded=Math.max(0,Math.min(integerRoot("presentation.maximum-particles-per-emission",128),requested));
        int viewDistance=integerRoot("presentation.view-distance-blocks",48);
        for(Player viewer:location.getWorld().getPlayers()){
            if(viewer.getLocation().distanceSquared(location)>viewDistance*viewDistance)continue;
            var settings=players.settings(viewer.getUniqueId());
            if(!settings.passiveParticles()||settings.effects()==com.dragonaltar.player.EffectMode.MINIMAL)continue;
            int count=settings.effects()==com.dragonaltar.player.EffectMode.REDUCED?Math.max(1,bounded/2):bounded;
            if(count>0)viewer.spawnParticle(particle,location,count,offsetX,offsetY,offsetZ,extra);
        }
    }
    ...
    static double boundedTargetRadius(double requested){
        if(!Double.isFinite(requested))return 0;
        return Math.max(0,Math.min(MAX_TARGET_RADIUS,requested));
    }
    static int maximumTargetsPerQuery(){return MAX_TARGETS_PER_QUERY;}
    ...
    private static double distanceSquaredToSegment(Vector point,Vector start,Vector end){
        ...
        if(lengthSquared<=.000001)return point.distanceSquared(start);
    ...
    private int integer(String path,int fallback){return config.file("abilities.yml").getInt("abilities."+path,fallback);}
    private int integerRoot(String path,int fallback){return config.file("abilities.yml").getInt(path,fallback);}
    private double decimal(String path,double fallback){return config.file("abilities.yml").getDouble("abilities."+path,fallback);}
    private double decimalRoot(String path,double fallback){return config.file("abilities.yml").getDouble(path,fallback);}
    private String label(String ability,String fallback){return config.file("abilities.yml").getString("abilities."+ability+".name",fallback);}
    private int resonanceInteger(String path,int fallback){return config.file("abilities.yml").getInt("resonances."+path,fallback);}
    private double resonanceDecimal(String path,double fallback){return config.file("abilities.yml").getDouble("resonances."+path,fallback);}
    private String resonanceLabel(String ability,String fallback){return config.file("abilities.yml").getString("resonances."+ability+".name",fallback);}
}
