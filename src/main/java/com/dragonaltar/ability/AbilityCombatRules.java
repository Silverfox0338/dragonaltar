...
public final class AbilityCombatRules {
	...
	public enum CombatDamageSource {
		DIRECT_PLAYER, PLAYER_PROJECTILE, FIRE_TICK, SCRIPTED_ABILITY, REFLECTION, OTHER
	}
	...
	/**
	 * Rev's hunt state. All time values are absolute milliseconds so the lifecycle
	 * and cap rules remain deterministic and independent of Bukkit.
	 */
	public static final class RevHuntTracker {
		...
		public boolean mark(UUID target, UUID caster, long nowMillis, long durationMillis, long maximumRemainingMillis,
				int maximumMarks) {
			prune(nowMillis);
			...
			if (existing != null && existing.caster().equals(caster)) {
				marks.put(target,
						new Mark(caster,
								Math.min(existing.maximumExpiresAtMillis(),
										Math.max(existing.expiresAtMillis(), expiry)),
								existing.maximumExpiresAtMillis()));
				return true;
			}
		...
		public void beginHunt(UUID caster, long nowMillis, long huntDurationMillis, long mobilityDurationMillis,
				long maximumMobilityDurationMillis) {
			HunterState state = hunters.computeIfAbsent(caster, ignored -> new HunterState());
		...
		public RampageGain gainRampage(UUID caster, UUID target, long nowMillis, int amount, int maximum,
				int perTargetLimit) {
			HunterState state = hunters.get(caster);
			if (state == null || !huntActive(caster, nowMillis) || amount <= 0 || maximum <= 0 || perTargetLimit <= 0
					|| state.rampageGains.getOrDefault(target, 0) >= perTargetLimit) {
		...
		public boolean consumeFinisher(UUID caster, long nowMillis, boolean validClaim, boolean resetHeat) {
			if (!finisherArmed(caster, nowMillis))
				return false;
	...
	public static double storedBulwarkDamage(double current, double preventedDamage, double storageFraction,
			double cap) {
		double addition = Math.max(0, preventedDamage) * clamp(storageFraction, 0, 1);
		return clamp(current + addition, 0, Math.max(0, cap));
	}

	public static double scaledBulwarkValue(double stored, double storageCap, double minimum, double maximum) {
		if (maximum < minimum) {
			return minimum;
		}
