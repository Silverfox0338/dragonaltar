...
public final class RevHeatBarManager {
    enum HeatTier { STALKING, PURSUING, PREDATOR }

    private final ConfigService config;
    private final Map<UUID, BossBar> bars=new HashMap<>();
    private final MiniMessage mini=MiniMessage.miniMessage();

    public RevHeatBarManager(ConfigService config){this.config=config;}

    public void update(Player player,int heat,int maximum,int mobilityThreshold,int trackingThreshold,boolean visible){
        if(!visible){hide(player);return;}
        ...
        BossBar bar=bars.computeIfAbsent(player.getUniqueId(),ignored->{
            BossBar created=BossBar.bossBar(net.kyori.adventure.text.Component.empty(),0,
                    BossBar.Color.YELLOW,overlay());
        ...
        bar.name(mini.deserialize(title
                .replace("<heat>",Integer.toString(boundedHeat))
                .replace("<maximum>",Integer.toString(boundedMaximum))
                .replace("<tier>",displayName(tier))));
    ...
    static HeatTier tier(int heat,int mobilityThreshold,int trackingThreshold){
        if(heat>=trackingThreshold)return HeatTier.PREDATOR;
        if(heat>=mobilityThreshold)return HeatTier.PURSUING;
    ...
    private BossBar.Color color(HeatTier tier){
        ...
        try{return BossBar.Color.valueOf(config.file("abilities.yml")
                .getString("rev-hunt.heat-bar."+path,fallback.name()).toUpperCase(Locale.ROOT));}
        catch(IllegalArgumentException ignored){return fallback;}
    }

    private BossBar.Overlay overlay(){
        try{return BossBar.Overlay.valueOf(config.file("abilities.yml")
                .getString("rev-hunt.heat-bar.overlay","PROGRESS").toUpperCase(Locale.ROOT));}
        catch(IllegalArgumentException ignored){return BossBar.Overlay.PROGRESS;}
    }
}
