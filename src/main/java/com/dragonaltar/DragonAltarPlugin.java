...
public final class DragonAltarPlugin extends JavaPlugin {
    ...
    @Override public void onEnable() {
        try {
            ...
            Bukkit.getOnlinePlayers().forEach(p->{dragonborn.apply(p);rituals.refundPending(p);
                if(!validateSetup().equals("Valid")&&p.hasPermission("dragonaltar.setup"))messages.send(p,"setup-incomplete");
                if(dragonEvent.state()==com.dragonaltar.dragonevent.DragonEventState.RECOVERY_REQUIRED&&p.hasPermission("dragonaltar.admin.event"))messages.send(p,"event-recovery-required");
            });
    ...
    public Set<String> integrations(){Set<String>s=new LinkedHashSet<>();for(String n:List.of("ScaledEnderDragon","PlaceholderAPI"))if(getServer().getPluginManager().isPluginEnabled(n))s.add(n);return s;}
    ...
    public boolean protectionConfigured(){Location first=configuredLocation("altar.yml","protection.pos1"),second=configuredLocation("altar.yml","protection.pos2");return first!=null&&second!=null&&first.getWorld().equals(second.getWorld());}
    public boolean protectionEnabled(){return configs.file("altar.yml").getBoolean("internal-protection.enabled",false);}
    public void setAltarValue(String path,Object value){YamlConfiguration y=configs.file("altar.yml");y.set(path,value);try{y.save(new File(getDataFolder(),"altar.yml"));configs.load();audit.record(path.startsWith("protection.")||path.startsWith("internal-protection.")?"PROTECTION_CHANGE":"ALTAR_CONFIG","SYSTEM",path+"="+value);}catch(IOException e){throw new IllegalStateException("Could not save altar configuration",e);}}
    public String validateSetup(){
        ...
        if(configs.file("altar.yml").getBoolean("internal-protection.required-for-event",false)){
            if(!protectionEnabled())errors.add("internal protection is required for the event but disabled");
            else if(!protectionConfigured())errors.add("internal protection is required for the event but its corners are incomplete or in different worlds");
        }
