...
public final class AdminGui implements Listener {
	...
	private final Component title = Component.text("DragonAltar Admin");
	private static final Map<String, Material> BUTTONS = Map.ofEntries(Map.entry("event", Material.END_CRYSTAL),
			Map.entry("altar", Material.DRAGON_EGG), Map.entry("ritual", Material.NETHER_STAR),
			Map.entry("souls", Material.ECHO_SHARD), Map.entry("dragonborn", Material.DRAGON_HEAD),
			Map.entry("abilities", Material.FEATHER), Map.entry("energy", Material.REDSTONE),
			Map.entry("protection", Material.SHIELD), Map.entry("integrations", Material.COMPARATOR),
			Map.entry("health", Material.HEART_OF_THE_SEA), Map.entry("beta", Material.COMMAND_BLOCK));
	public AdminGui(DragonAltarPlugin plugin) {
		this.plugin = plugin;
		buttonKey = new NamespacedKey(plugin, "admin_button");
	}
	public void open(Player player) {
		...
		for (var entry : BUTTONS.entrySet()) {
			...
			meta.displayName(Component.text(capitalize(entry.getKey())));
			meta.getPersistentDataContainer().set(buttonKey, PersistentDataType.STRING, entry.getKey());
	...
	public void click(InventoryClickEvent e) {
		...
		if (!(e.getWhoClicked() instanceof Player p) || e.getCurrentItem() == null)
			return;
		String action = e.getCurrentItem().getItemMeta().getPersistentDataContainer().get(buttonKey,
		...
		switch (action) {
			...
			case "beta" -> {
				if (plugin.configService().serverMode().name().equals("PRODUCTION"))
					p.sendMessage("Beta tools are disabled in production.");
				else
					dispatch(p, "dragon dev perf status");
			}
