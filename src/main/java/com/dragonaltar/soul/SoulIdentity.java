...
public enum SoulIdentity {
    ...
    public static SoulIdentity fromId(String id) {
        for (SoulIdentity identity : values()) if (identity.id.equalsIgnoreCase(id)) return identity;
        throw new IllegalArgumentException("Unknown soul: " + id);
    }

    public static String displayName(String id) {
        try { return fromId(id).displayName; }
        catch (IllegalArgumentException ignored) { return id == null ? "Soul" : id.toLowerCase(Locale.ROOT); }
    }

    public static String replaceIds(String text) {
        if (text == null) return "";
        String result = text;
        for (SoulIdentity identity : values()) result = result.replace(identity.id, identity.displayName);
        return result;
    }
}
