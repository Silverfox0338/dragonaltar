...
public final class DragonCommand implements TabExecutor {
    ...
    private boolean event(CommandSender s, String[] a) {
        ...
        if(sub.equals("rescan")){danger(s,"event-rescan",List.of(),()->s.sendMessage(plugin.dragonEvent().rescan()));return true;}
        if(sub.equals("recover")){danger(s,"event-recover",List.of(),()->s.sendMessage(plugin.dragonEvent().recover()));return true;}
    ...
    private boolean setup(CommandSender s,String[] a) {
        ...
        if(sub.equals("save")){danger(s,"setup-save",List.of(p.getUniqueId().toString()),()->{plugin.setup().save(p);if(plugin.validateSetup().equals("Valid")){plugin.dragonEvent().setAltarState(com.dragonaltar.altar.AltarState.CONFIGURED);plugin.ensureRuntimeTasks();}s.sendMessage("Setup saved. "+plugin.validateSetup());});return true;}
    ...
    private boolean altar(CommandSender s,String[] a) {
        ...
        if(sub.equals("preview")){Location l=plugin.configuredLocation("altar.yml","altar-center");if(l==null)throw new IllegalStateException("Altar center unconfigured");plugin.animations().play("altar-awaken",l,s instanceof Player p?p:null);return true;}
        if(sub.equals("awaken")){requireForcedStateAllowed();Location l=plugin.configuredLocation("altar.yml","altar-center");if(l==null)throw new IllegalStateException("Altar center unconfigured");danger(s,"altar-activate",List.of(),()->{plugin.animations().play("altar-awaken",l,s instanceof Player p?p:null);plugin.dragonEvent().setAltarState(com.dragonaltar.altar.AltarState.ACTIVE);});return true;}
        if(sub.equals("deactivate")||sub.equals("dormancy")){requireForcedStateAllowed();danger(s,"altar-dormancy",List.of(),()->{plugin.dragonEvent().setAltarState(com.dragonaltar.altar.AltarState.DORMANT);plugin.displays().previewCount(0);plugin.displays().remove();});return true;}
        if(sub.equals("activate")){requireForcedStateAllowed();danger(s,"altar-activate",List.of(),()->{plugin.dragonEvent().setAltarState(com.dragonaltar.altar.AltarState.ACTIVE);plugin.displays().resetPreview();plugin.displays().recover(plugin.configuredLocation("altar.yml","egg-display"));});return true;}
        if(sub.equals("egg")&&a.length>2){Location l=plugin.configuredLocation("altar.yml","egg-display");switch(a[2]){case "spawn"->{plugin.displays().previewCount((int)Math.max(1,plugin.souls().unclaimedCount()));plugin.displays().recover(l);}case "reset","teleport-to-config"->{plugin.displays().resetPreview();plugin.displays().recover(l);plugin.displays().resetTransformation();}case "remove"->danger(s,"altar-egg-remove",List.of(),()->{plugin.displays().previewCount(0);plugin.displays().remove();});case "inspect"->s.sendMessage(plugin.displays().inspect());case "animate"->{if(a.length<4)throw new IllegalArgumentException("Animation required");String animation=Map.of("idle","egg-idle","awaken","altar-awaken","claim","egg-claim","deplete","egg-deplete").getOrDefault(a[3],a[3]);plugin.animations().play(animation,l,s instanceof Player p?p:null);}case "count"->{if(a.length<4)throw new IllegalArgumentException("Count 0-3 required");plugin.displays().previewCount(Integer.parseInt(a[3]));plugin.displays().recover(l);}default->throw new IllegalArgumentException("Unknown egg action");}return true;}
    ...
    private boolean ritual(CommandSender s,String[] a) {
        ...
        if(sub.equals("start")&&a.length>2){Player target=requiredOnline(a[2]);danger(s,"ritual-force-start",List.of(target.getUniqueId().toString()),()->plugin.rituals().start(target));return true;}
        if(sub.equals("stop")||sub.equals("fail")||sub.equals("refund")){var active=plugin.rituals().active().orElseThrow(()->new IllegalStateException("No ritual is active"));danger(s,"ritual-cancel",List.of(active.playerId().toString(),active.soulId()),()->plugin.rituals().cancel(true));return true;}
        ...
        if(sub.equals("reserve")&&a.length>3){Player target=requiredOnline(a[3]);danger(s,"ritual-reserve",List.of(a[2],target.getUniqueId().toString()),()->plugin.souls().reserve(a[2],target.getUniqueId()));return true;}
        if(sub.equals("release")&&a.length>2){DragonSoul soul=plugin.souls().byId(a[2]).orElseThrow(()->new IllegalArgumentException("Unknown soul"));danger(s,"ritual-release",List.of(soul.id(),String.valueOf(soul.reservedFor())),()->plugin.souls().release(soul.id()));return true;}
    ...
    private boolean admin(CommandSender s,String[] a) {
        ...
        if(a[1].equals("cooldown")&&a.length>3){Player target=requiredOnline(a[3]);if(a[2].equals("view"))s.sendMessage(plugin.abilities().cooldowns(target).toString());else if(a[2].equals("clear")){String ability=a.length>4?a[4]:"all";danger(s,"cooldown-clear",List.of(target.getUniqueId().toString(),ability),()->plugin.abilities().clearCooldowns(target,ability));}return true;}
    ...
    private boolean system(CommandSender s,String[] a) {
        ...
        else if(sub.equals("cleanup")){danger(s,"system-cleanup",List.of(),()->{plugin.dragonEvent().clearTestDragons();Location egg=plugin.configuredLocation("altar.yml","egg-display");if(egg!=null)plugin.displays().recover(egg);s.sendMessage("Duplicate display and test-entity cleanup completed.");});}
    ...
    @Override public List<String> onTabComplete(CommandSender s,Command c,String l,String[] a) {
        if(a.length==1)return filter(List.of("status","history","refunds","abilities","focus","settings","help","event","setup","altar","ritual","admin","soul","protection","system","dev","confirm","cancel"),a[0]);
        ...
        if(a.length==2&&a[0].equalsIgnoreCase("soul"))return filter(List.of("force-remove","force-transfer"),a[1]);
        ...
        if(a.length==3&&a[0].equalsIgnoreCase("soul")&&a[1].equalsIgnoreCase("force-remove"))return filter(Bukkit.getOnlinePlayers().stream().map(Player::getName).toList(),a[2]);
        if(a.length==4&&a[0].equalsIgnoreCase("soul")&&a[1].equalsIgnoreCase("force-remove"))return filter(Arrays.stream(SoulIdentity.values()).map(SoulIdentity::displayName).toList(),a[3]);
        if(a.length==3&&a[0].equalsIgnoreCase("soul")&&a[1].equalsIgnoreCase("force-transfer"))return filter(Arrays.stream(SoulIdentity.values()).map(SoulIdentity::displayName).toList(),a[2]);
        if(a.length>=4&&a[0].equalsIgnoreCase("soul")&&a[1].equalsIgnoreCase("force-transfer"))return filter(Bukkit.getOnlinePlayers().stream().map(Player::getName).toList(),a[a.length-1]);
        return List.of();
    }
    ...
    private void danger(CommandSender sender,String operation,List<String> arguments,Runnable action){
        UUID id=senderId(sender);Duration duration=confirmationDuration();
        String token=plugin.confirmations().issue(id,operation,arguments,duration);
        DestructiveActionPreview preview=destructivePreview(operation,arguments);
        dangerous.put(id,new PendingDanger(operation,List.copyOf(arguments),preview,stateFingerprint(operation,arguments),action));
        plugin.audit().record("DESTRUCTIVE_CONFIRMATION_ISSUED",id.toString(),
                "admin="+sender.getName()+" operation="+operation+" arguments="+arguments+" expires="+duration.toSeconds()+"s");
        sender.sendMessage(preview.render(token,duration.toSeconds()));
    }
    private void confirmDanger(CommandSender sender,String[] args){
        if(args.length<2)throw new IllegalArgumentException("Confirmation token required");
        UUID id=senderId(sender);PendingDanger pending=dangerous.remove(id);
        if(pending==null||!plugin.confirmations().consume(id,args[1],pending.operation,pending.arguments)){
            plugin.audit().record("DESTRUCTIVE_CONFIRMATION_REJECTED",id.toString(),"admin="+sender.getName()+" invalid-or-expired");
            throw new IllegalArgumentException("Invalid or expired confirmation; no changes were made");
        }
        if(!pending.stateFingerprint.equals(stateFingerprint(pending.operation,pending.arguments))){
            plugin.audit().record("DESTRUCTIVE_CONFIRMATION_REJECTED",id.toString(),
                    "admin="+sender.getName()+" operation="+pending.operation+" state-changed-after-preview");
            throw new IllegalStateException("Relevant server state changed after the preview; no changes were made. Review and run the original command again");
        }
        try {
            pending.action.run();
            plugin.audit().record("DESTRUCTIVE_ACTION_CONFIRMED",id.toString(),
                    "confirmed-by="+sender.getName()+" operation="+pending.operation+" arguments="+pending.arguments
                            +" result="+pending.preview.result());
            sender.sendMessage("Confirmed by "+sender.getName()+": "+pending.preview.action()+" completed.");
        } catch(RuntimeException ex) {
            plugin.audit().record("DESTRUCTIVE_ACTION_FAILED",id.toString(),
                    "confirmed-by="+sender.getName()+" operation="+pending.operation+" error="+String.valueOf(ex.getMessage()));
            throw ex;
        }
    }
    private void cancelDanger(CommandSender sender){
        UUID id=senderId(sender);PendingDanger pending=dangerous.remove(id);boolean token=plugin.confirmations().cancel(id);
        if(pending==null&&!token){sender.sendMessage("No destructive action is awaiting confirmation.");return;}
        plugin.audit().record("DESTRUCTIVE_CONFIRMATION_CANCELLED",id.toString(),
                "admin="+sender.getName()+" operation="+(pending==null?"unknown":pending.operation));
    ...
    private DestructiveActionPreview destructivePreview(String operation,List<String> args){
        ...
        switch(operation){
            case "admin-grant"->{
                players.add(playerName(args.get(0)));souls.add(soulName(args.get(1)));
                result=souls.getFirst()+" will become Held by "+players.getFirst()+".";
                history="A permanent grant event will be appended.";undo="Can be reversed with another confirmed transfer; the audit/history entry remains.";
            }
            case "soul-assign"->{
                souls.add(soulName(args.get(0)));players.add(playerName(args.get(1)));
                result=souls.getFirst()+" will become Held by "+players.getFirst()+".";
                history="A permanent grant event will be appended.";undo="Can be reversed with another confirmed transfer; the audit/history entry remains.";
            }
            case "admin-remove"->{
                players.add(playerName(args.getFirst()));
                plugin.souls().byHolder(UUID.fromString(args.getFirst())).ifPresent(soul->souls.add(soulName(soul.id())));
                result=(souls.isEmpty()?"The player's soul":souls.getFirst())+" will leave "+players.getFirst()+" and enter dormant/pending reincarnation.";
                history="Loss and removal events will be appended.";undo="Can be reassigned with a confirmed grant; history remains.";
            }
            case "admin-transfer"->{
                players.add(playerName(args.get(0)));players.add(playerName(args.get(1)));
                plugin.souls().byHolder(UUID.fromString(args.get(0))).ifPresent(soul->souls.add(soulName(soul.id())));
                result=(souls.isEmpty()?"The held soul":souls.getFirst())+" will transfer from "+players.get(0)+" to "+players.get(1)+".";
                history="Permanent loss, gain, and transfer events will be appended.";undo="Can be transferred back with another confirmation; history remains.";
            }
            case "admin-transfer-soul"->{
                souls.add(soulName(args.get(0)));players.add(playerName(args.get(1)));
                String old=plugin.souls().byId(args.get(0)).map(DragonSoul::holder).map(this::playerName).orElse("no current holder");
                result=souls.getFirst()+" will transfer from "+old+" to "+players.getFirst()+".";
                history="Permanent ownership events will be appended.";undo="Can be transferred back with another confirmation; history remains.";
            }
            case "admin-make-pending","soul-unassign"->{
                souls.add(soulName(args.getFirst()));
                plugin.souls().byId(args.getFirst()).map(DragonSoul::holder).ifPresent(id->players.add(playerName(id)));
                result=souls.getFirst()+" will lose its holder and enter dormant/pending reincarnation.";
                history="A permanent pending/removal event will be appended.";undo="Can be reassigned with a confirmed command; history remains.";
            }
            case "admin-reincarnate"->{
                souls.add(soulName(args.get(0)));players.add(playerName(args.get(1)));
                result=souls.getFirst()+" will immediately become Held by randomly selected "+players.getFirst()+".";
                history="A permanent reincarnation event will be appended.";undo="Can be transferred again; history remains.";
            }
            case "ritual-reserve"->{
                souls.add(soulName(args.get(0)));players.add(playerName(args.get(1)));
                result=souls.getFirst()+" will be reserved for "+players.getFirst()+" and unavailable to other rituals.";
                history="Unchanged.";undo="Reversible with /dragon ritual release "+args.get(0)+".";
            }
            case "ritual-release"->{
                souls.add(soulName(args.get(0)));if(args.size()>1&&!args.get(1).equals("null"))players.add(playerName(args.get(1)));
                result=souls.getFirst()+" will return to dormant/unclaimed and its ritual reservation will be discarded.";
                history="Unchanged.";undo="The soul can be reserved again.";
            }
            case "ritual-force-start"->{
                players.add(playerName(args.getFirst()));
                result="A ritual will start for "+players.getFirst()+" and exact offerings may be consumed.";
                history="A ritual start is audited; ownership history changes only if it completes.";undo="The ritual can be cancelled with a separate confirmation; offerings are then refunded.";
            }
            case "ritual-cancel"->{
                players.add(playerName(args.get(0)));souls.add(soulName(args.get(1)));
                result="The active ritual will stop, "+souls.getFirst()+" will be released, and consumed offerings will be refunded or queued.";
                history="Soul ownership history is unchanged; cancellation is audited.";undo="The ritual cannot resume, but the player can start a new one.";
            }
            case "ritual-force-complete"->{
                plugin.rituals().active().ifPresent(active->{players.add(playerName(active.playerId()));souls.add(soulName(active.soulId()));});
                ...
                history="A permanent claim or fracture event will be appended.";undo="Not automatically reversible; restore a pre-action backup for an exact rollback.";
            }
            case "event-abort"->{result="The official event will be marked ABORTED and protected event crystals will be removed.";undo="Requires recovery/manual restart or a matching backup restore.";}
            case "event-rescan"->{result="Loaded dragons/crystals will be rescanned; canonical dragon and event recovery state may be rewritten.";undo="Restore a matching backup if the recovered identity/state is wrong.";}
            case "event-recover"->{result="Blackout recovery will choose a state from persisted souls and loaded event entities; it may activate/dormant the altar or abort the event.";undo="Restore a matching pre-recovery backup for an exact rollback.";}
            case "altar-activate"->{result="The altar state becomes ACTIVE and its displays are restored.";undo="Reversible with a separately confirmed dormancy command.";}
            case "altar-dormancy"->{result="The altar state becomes DORMANT and altar displays are removed.";undo="Reversible with a separately confirmed activate command.";}
            case "altar-egg-remove"->{result="Plugin-owned altar egg displays will be removed; soul data is unchanged.";undo="Reversible with /dragon altar egg spawn.";}
            case "altar-recipe-remove"->{result="The altar recipe display will be removed; ritual requirements are unchanged.";undo="Reversible with /dragon altar recipe spawn.";}
            case "altar-recipe-move"->{result="The saved recipe display offset will be overwritten to place it at "+args.getFirst()+".";undo="Move it back with another confirmed command or restore the configuration.";}
            case "setup-save"->{result="All staged setup locations will overwrite the saved altar configuration.";undo="Restore a configuration backup or stage the prior locations again.";}
            case "protection-disable"->{result="Internal altar protection will be disabled; configured region coordinates remain.";undo="Reversible with /dragon protection enable.";}
            case "protection-setpos1","protection-setpos2"->{result="The saved protection corner will be overwritten with "+args.getFirst()+".";undo="Set the prior coordinate with another confirmed command or restore the configuration.";}
            case "cooldown-clear"->{
                players.add(playerName(args.get(0)));result="Stored "+args.get(1)+" cooldowns for "+players.getFirst()+" will be cleared.";
                cooldowns="Cleared for "+args.get(1)+".";undo="Cannot reconstruct previous expiry times without a backup.";
            }
            case "dev-recipe-consume"->{
                players.add(playerName(args.getFirst()));result="Exact configured ritual offerings will be removed from "+players.getFirst()+" and queued as a diagnostic refund.";
                undo="Recoverable with the recipe-refund diagnostic.";
            }
            case "promote-test-dragon"->{result="Test dragon "+args.getFirst()+" will become the official canonical event dragon.";undo="Not automatically reversible; use a matching backup and inspect entities.";}
            case "clear-test-dragons"->{result="All loaded plugin-tagged test dragons will be permanently removed.";undo="Removed entities cannot be restored.";}
            case "clear-event-crystals"->{result="Official and test event crystals will be permanently removed.";undo="Removed entities cannot be restored.";}
            case "system-cleanup"->{result="Loaded test dragons will be removed and plugin displays reconciled to configuration.";undo="Removed test entities cannot be restored; displays can be regenerated.";}
            case "soul-delete"->{
                souls.add(soulName(args.getFirst()));plugin.souls().byId(args.getFirst()).map(DragonSoul::holder).ifPresent(id->players.add(playerName(id)));
                result=souls.getFirst()+" and its complete ownership lineage will be deleted.";
                history="That soul's complete history is deleted.";undo="Only a matching data backup can restore it.";
            }
            case "soul-setstate"->{
                souls.add(soulName(args.get(0)));plugin.souls().byId(args.get(0)).map(DragonSoul::holder).ifPresent(id->players.add(playerName(id)));
                result=souls.getFirst()+" will be forced to "+args.get(1)+", potentially removing its holder.";
                history="A permanent forced-state event may be appended.";undo="Requires another confirmed state/assignment change; history remains.";
            }
            case "restore-backup"->{
                players.add("All persisted players");souls.add("All Dragon Souls");result="Backup "+args.getFirst()+" will overwrite event, soul, player, ritual, cooldown, pending-transfer, and consequence data.";
                cooldowns="Replaced by the selected backup.";history="Replaced by the selected backup.";undo="Only possible if a newer backup was created before this restore.";
            }
            case "reset-souls"->{souls.add("All Dragon Souls");players.add("All current holders");result="Every soul record and holder binding will be deleted.";history="All soul lineage is deleted.";undo="Only a matching backup can restore it.";}
            case "reset-event"->{result="Official event state, canonical entity reference, timestamps, and event crystals will be reset.";history="Player/soul history is unchanged.";undo="Only a matching backup can restore exact event state.";}
            case "reset-players"->{
                players.add("All persisted players");result="All player settings, energy, ability selections, cooldowns, and player history will be deleted.";
                cooldowns="All deleted.";history="All player gain/loss history deleted; soul lineage remains.";undo="Only a matching backup can restore it.";
            }
            case "reset-altar"->{result="All saved altar, fountain, crystal, pedestal, and protection locations will be deleted.";undo="Only a configuration/data backup or complete setup can restore it.";}
            case "reset-history"->{players.add("All persisted players");souls.add("All Dragon Souls");result="Every soul lineage and player Dragonborn history entry will be deleted.";history="Permanently wiped.";undo="Only a matching backup can restore it.";}
            case "reset-everything"->{
                players.add("All persisted players");souls.add("All Dragon Souls");result="Event, altar, souls, consequences, rituals, player data, cooldowns, and history will be reset.";
                cooldowns="All deleted.";history="All deleted.";undo="Only a full matching backup can restore this data.";
            }
            default->{result="Operation "+operation+" will mutate persistent or world state using arguments "+args+".";undo="Review backups before confirming.";}
        }
        return new DestructiveActionPreview(operation.replace('-',' '),players,souls,result,cooldowns,history,undo);
    }
    private String stateFingerprint(String operation,List<String> arguments){
        StringBuilder state=new StringBuilder();
        plugin.souls().all().forEach(soul->state.append(soul.id()).append(':').append(soul.state()).append(':')
                .append(soul.holder()).append(':').append(soul.reservedFor()).append(':').append(soul.lineage().size()).append('|'));
        state.append("event=").append(plugin.dragonEvent().state()).append(':').append(plugin.dragonEvent().sessionId())
                .append(':').append(plugin.dragonEvent().dragonId()).append(':').append(plugin.dragonEvent().altarState());
        state.append("|ritual=").append(plugin.rituals().active().map(active->
                active.sessionId()+":"+active.playerId()+":"+active.soulId()+":"+active.phase()).orElse("none"));
    ...
    private String playerName(String value){
        try{return playerName(UUID.fromString(value));}catch(IllegalArgumentException ignored){return value;}
    }
